    ...
    private inner class UnlockClockWallpaperEngine(val context: Context) : Engine() {
        ...
        private fun spawnUnlockParticles() {
            ...
            val density = runCatching {
                val raw = prefs.getString(PreferenceKeys.KEY_PARTICLE_DENSITY, null)
                raw?.toIntOrNull() ?: prefs.getInt(PreferenceKeys.KEY_PARTICLE_DENSITY, 40)
            }.getOrDefault(40).coerceIn(0, 120)
            repeat(density) {
                particles.add(
                    Particle(
                        x = centerX,
